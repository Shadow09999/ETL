...
import org.example.multitask.MultiTaskMultiThread;
import org.example.multitask.Task;
import org.example.multitask.multithread.NTaskPerThread;
import org.example.multitask.task.Table2TableTask;
import org.example.utils.ConfigParserUtils;
import org.example.utils.ConfigBuilderUtils;
...
public class DatabaseHelper {
    static final Logger logger = LoggerFactory.getLogger(org.example.database.DatabaseHelper.class);
            ...
            if (dropandcreate) {
                ...
                for (String tableName : tablesDDL.keySet()) {
                    if (destTables.contains(tableName)) {
                        org.example.database.DatabaseHelper.executeUpdate(destConn, getDropTableSQL(destDatabaseType, tableName));
                        logger.info("@@@ 删除表 {} ", tableName);
                    }
                    org.example.database.DatabaseHelper.executeUpdate(destConn, tablesDDL.get(tableName));
